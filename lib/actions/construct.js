...
export function executeRetargetConstructActionForEncounter(body = {}, deps = {}) {
  const { resolveActor, findTargetableEntity, pushLog, touchState, broadcastState } = deps;
  const participant = resolveActor(body.participantId);
  if (!participant) {
    return { error: 'Participant required' };
  }
...
export function executeMoveConstructActionForEncounter(body = {}, deps = {}) {
  ...
  const baseMove = Math.max(0, Math.round(Number(construct.moveFt || 10)));
  const difficultTerrain = Boolean(body.difficultTerrain);
  const distance = difficultTerrain ? Math.max(0, Math.floor(baseMove / 2)) : baseMove;
  if (distance <= 0) {
    return { error: `${construct.name} cannot move.` };
  }
